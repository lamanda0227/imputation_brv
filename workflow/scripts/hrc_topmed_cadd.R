...
annot_fname = sprintf("~/project/imputation-rvtest/analysis/imputation_aggregated_analysis/hrc_topmed/hrc_topmed_168206ids_rsq0%d_maf%s_cadd_annot.csv.gz", rsq, maf_c)
...
for(g in genes[c(from:to)]){
    ...
    if(exists("bed_full")){
    	fwrite(bed_full, 
           	sprintf("/home/tl3031/project/imputation-rvtest/workflows/imputation_aggregated_analysis/maf%s/hrc_topmed_cadd_rsq0%d/%s.traw", 
                   	maf_c, rsq, fname))
    	rm(bed_full)
    }
}

...
annot_fname = sprintf("~/project/imputation-rvtest/analysis/imputation_aggregated_analysis/hrc_topmed_exome/hrc_topmed_exome_168206ids_rsq0%d_maf%s_annot.csv.gz", rsq, maf_c)
...
for(g in genes[c(from:to)]){
    ...
    print(fname)
    print(dim(annot %>% filter(Gene.refGene == g)))
    ...
    if(file.exists(topmed_fname)){
        ...
        print("topmed snplist")
        print(length(topmed_snplist))
        print("topmed mat")
        print(dim(topmed_gene))
        if(exists("bed_full")){
    ...
    fwrite(bed_full, 
           sprintf("/home/tl3031/project/imputation-rvtest/workflows/imputation_aggregated_analysis/maf%s/hrc_topmed_exome_rsq0%d/%s.traw", 
                   maf_c, rsq, fname))
}

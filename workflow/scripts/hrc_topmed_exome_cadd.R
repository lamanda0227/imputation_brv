...
exome_folder = sprintf("~/project/imputation-rvtest/workflows/imputation_aggregated_analysis/maf%s/exome_cadd", maf_c)
hrc_folder = sprintf("~/project/imputation-rvtest/workflows/imputation_aggregated_analysis/maf%s/hrc_cadd_rsq0%d", maf_c, rsq)
topmed_folder = sprintf("~/project/imputation-rvtest/workflows/imputation_aggregated_analysis/maf%s/topmed_cadd_rsq0%d", maf_c, rsq)

annot_fname = sprintf("~/project/imputation-rvtest/analysis/imputation_aggregated_analysis/hrc_topmed_exome/hrc_topmed_exome_168206ids_rsq0%d_maf%s_cadd_annot.csv.gz", rsq, maf_c)
...
for(g in genes[c(from:to)]){
    ...
    exome_file <- list.files(exome_folder, pattern = paste0(fname,".bim"))
    if(length(exome_file) != 0){
        ...
        bed_full <- bed[, order(colnames(bed))]
        if(is.vector(bed_full)){
            colnames <- names(as.list(bed_full))
            bed_full <- as.data.frame(as.list(bed_full))
            colnames(bed_full) <- colnames
        }
    }
    ...
    if(file.exists(hrc_fname)){
        ...
        if(exists("bed_full")){
            bed_full <- rbind(bed_full, hrc_gene)
        } else {
            bed_full <- hrc_gene
        }
    }
    ...
    fwrite(bed_full, 
           sprintf("/home/tl3031/project/imputation-rvtest/workflows/imputation_aggregated_analysis/maf%s/hrc_topmed_exome_cadd_rsq0%d/%s.traw", 
                   maf_c, rsq, fname))
}

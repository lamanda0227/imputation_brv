...
for(gene_idx in c(gene_start:gene_end)){
    ...
    gene <- parse_input$gene
    gene <- gsub("chr1_", "", gene)
    gene <- gsub("chr2_", "", gene)

    exome_fname <- sprintf("~/project/imputation-rvtest/workflows/imputation_aggregated_analysis/gene/exome_topmed_overlap/exome_%s/%s.raw", maf_c, gene)
    if(!file.exists(exome_fname)) exome_fname <- NA

    topmed_fname <- sprintf("~/project/imputation-rvtest/workflows/imputation_aggregated_analysis/gene/exome_topmed_overlap/topmed_%s/%s.raw", maf_c, gene)

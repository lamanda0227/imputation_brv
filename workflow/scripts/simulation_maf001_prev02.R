...
result_filtered <- result %>% filter(fixed_effect_prop.deletrious_effect == 1.5, fixed_effect_prop.causal_del == "c(1,1)", bin_phenotype.prevalence == 0.2)
...
for(i in c(start:end)){
    ...
    if(file.exists(bim_fname)){ 
        ...
        exome_result <- BRV(exome_X, y, "exome", 1.5, 1, 0.2)
    } else {
    ...
    topmed_result <- BRV(topmed_X, y, "topmed", 1.5, 1, 0.2)

...
generate_fixed_effect_prop <- function(bim_fname, hrc_topmed_exome_fname, option, deletrious_effect, protective_effect, causal_prop, deleterious_prop){
    if(option == "bim"){
        bim <- read_bim(bim_fname)
        num_snp <- nrow(bim)
    }else{
        geno_mat <- data.table::fread(hrc_topmed_exome_fname, header = TRUE)
        num_snp <- nrow(geno_mat)
    }
